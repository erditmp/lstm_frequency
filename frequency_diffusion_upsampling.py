...
# Path to CSV dataset containing minute level measurements
DATA_PATH = "reduced_dataset_first_1percent.csv"
...
if __name__ == "__main__":
    ...
    dataset = WindTurbineFrequencyDataset(df, upsample_steps=UPSAMPLE_STEPS,
    ...
    # Compare predicted vs original for the first hour gap
    example_hourly = dataset.hourly_data.iloc[:2].copy()
    ...
    if len(upsampled) > 0:
        for idx, feature in enumerate(feature_cols):
            ...
            plt.plot([t0, t1], example_hourly[feature].values, "ro", label="Hourly")
            minutes = [t0 + timedelta(minutes=i + 1) for i in range(UPSAMPLE_STEPS)]
            plt.plot(minutes, upsampled[0][:, idx], "b-", label="Predicted minutes")
            actual = dataset.data[(dataset.data["time"] > t0) &
                                  (dataset.data["time"] <= t1)].head(UPSAMPLE_STEPS)
            if len(actual) == UPSAMPLE_STEPS:
                plt.plot(actual["time"], actual[feature], "g--", label="Actual minutes")
            plt.legend()
            plt.xlabel("Time")
            plt.ylabel(feature)
            plt.title(f"Frequency Diffusion Upsampling for {feature}")
            plt.grid(True)
            plt.tight_layout()
            plt.show()
